...
SELECT country.country, customer.last_name, SUM(payment.amount) AS facturetotale
FROM country
JOIN city USING (country_id) 
JOIN address USING (city_id) 
JOIN customer USING (address_id)
...
SELECT category.name, AVG(film.length) AS longueurmoyenne FROM film
